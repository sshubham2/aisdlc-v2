...
Claims not backed by evidence: "endpoint enforces authz" but not *how*; "library X handles HEIC" with no spike; "async queue is sufficient" with no load estimate. **Tool-doc-vs-implementation parity:** when design.json cites an existing audit/tool's expected format, OPEN the tool source (`scripts/lib/*.py` or `skills/<name>/scripts/*.py`) and confirm the docstring example matches the actual regex/keyword-list — docstrings drift. **Phantom-import:** a new `.py` importing a name that doesn't exist / was renamed. **Spike-evidence walk:** design.json's optional `assumptions_proven[]` is your assumption→evidence index — an assumption the design treats as settled should appear there with a `spike_ref`; one claimed-proven WITHOUT a spike_ref is a finding in this dimension. An ABSENT block means "no spiked assumptions", never an error — and it does NOT excuse unspiked load-bearing premises from this dimension's normal scrutiny.
