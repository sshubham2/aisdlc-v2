...
Interface reference for the `ai-sdlc` Claude Code plugin, **v2.40.0**. This is a **reference**, not a tutorial —
...
| `/ai-sdlc:setup` | `[--no-mcp] [--no-graph] [repo-path]` | One-shot dependency doctor. Installs PyYAML + `code-review-graph`, registers the CRG MCP server, scaffolds `.aisdlc/reality-gates.json` (id-merging the `bandit`/`pip-audit` security gates on a Python frame + vendoring the guard to `.aisdlc/gates/py_security_gate.py`), builds the code graph. Idempotent. `--no-mcp` skips MCP registration, `--no-graph` skips the initial graph build. On a git repo it also offers two further consented steps: pinning the repo's vault and picking a vault **sync backend** (`local`/`git`/`s3` — §2, `vault_admin.py set-backend`/`set-base`); both run after the deps install and never block it. | n/a — out-of-loop maintenance |
...
| Command | Arguments | Purpose | Auto-advance |
|---|---|---|---|
| `/slice` | `["<description or hint>"] [--area <NAME>\|--component <NAME>]` | Reads the ranked `candidates.json` backlog (read-only pick path), presents a ranked recommendation, claims the pick (worktree + branch + `pick_log`), writes `mission-brief.json` + `milestone.json`. **`--area <NAME>`** (deprecated alias `--component`, slice-080/084, population widened slice-098) scopes the ranked recommendation to ONE product area (`unassigned` for un-grouped product capabilities) — a read-only LENS on `candidates_top.py`'s digest: it takes no lock, mints no id, writes no status, and blocked/in-flight candidates stay visible globally. `/slice` also surfaces `product_rollup.py`'s per-area capability-progress rollup and biases the recommendation toward a product-sourced capability whenever it carries a non-empty `governor` (the product's scope is decomposed but 0 capabilities are built). An OPT-IN **completion gate** (`candidates_top.py --completion-gap`, slice-102/SC-232) additionally classifies the whole pick surface for **app completion** — `product-work-available` / `scope-exhausted` / `scope-absent` — and, once the product's initial scope is exhausted, routes the recommendation toward finishing a real unbuilt capability (or the correct next step: `/discover`, `/slice-candidates --add-item`, coordinating with an in-flight owner, repairing torn provenance) rather than silently falling through to score-ranked pipeline exhaust; the decline back to the ordinary ranked pick is always offered. Requires a failing `/repro` test first for bug-fix candidates (BFRD-1). | yes → `/risk-spike` |
...
| `/validate-slice` | `[slice-id]` | Forked context. Per-criterion PASS/FAIL/PARTIAL checks on the **real environment**. Runs VAL-1/WS-1/ETC-1 layered audits, the shippability-catalog regression check (run **synchronously**, inline in the forked context, so its verdict is never orphaned — slice-094 / SC-186), and the project's declared reality gates (`reality_gate_runner.py --repo-root <worktree> --json`, fail-closed — an absent/empty manifest is a structural no-op). | conditional — aggregate `Result: PASS` only |
| `/reflect` | `[slice-id]` | Captures validated / corrected / discovered / deferred learnings. Writes `reflection.json` (Critic-calibration findings under the canonical `critic_calibration` key — slice-101), appends `lessons-learned.json` + `shippability.json`, tracks Critic calibration, auto-archives the slice. | **false — the auto-advance terminus** |
...
| `migrate` | `--vault <path>`, `--reverse` | Convert the flat `gate-log.json` to the append-only shard store + a derived, git-ignored cache (`--reverse` rebuilds the flat file and tears the shards down; slice-088/ADR-106). Fail-closed, reversible, idempotent; holds the gate-log lock across the whole read→build→verify→publish, so a parallel-slice append can never be lost or duplicated. |
| `read-entries` | `--vault <path>`, `--rel-key <file>` (default `gate-log.json`), `--array <key>` (default/only `entries`) | Print a (possibly sharded) aggregate's entries as JSON, deriving them from the shard log when the derived cache is missing — e.g. right after a fresh `sync pull` (slice-089/SC-194). |
| `sync` | `push\|pull` (positional), `--vault`, `--backend {local,git,s3}` (default: the persisted `/setup` choice in `sync-backend.json`, else `git` for back-compat), `--remote`, `--force`, `--force-drop-local`, `--s3-bucket`, `--s3-endpoint-url`, `--s3-prefix` | Push/pull the vault's sync-set over a git remote or an S3/MinIO bucket (slice-092/095). `push` = stage → commit → push (git) or upload (s3). `pull` = fast-forward-only by default (git) — REFUSES a dirty tree or divergent history (`--force` mirror-resets, still guarding unpushed local commits unless `--force-drop-local`) — or overwrites a conflicting local artifact (s3, `--force`). **Transmits the whole vault UNREDACTED** — use a private remote / a Block-Public-Access bucket. |
| `set-backend` | `--backend {local,git,s3}` (required), `--remote` (git), `--s3-bucket`/`--s3-endpoint`/`--s3-region`/`--s3-project` (s3), `--vault` | Record `/ai-sdlc:setup`'s chosen sync backend + its non-secret config to `<git-common-dir>/aisdlc/sync-backend.json`, read-back-verified (slice-097/ADR-121+123). Never prompts for or persists a credential; an S3 endpoint embedding userinfo (`user:pass@host`) is refused (exit 2). Choosing `git` also arms the opt-in git-backed claim register (see `claim_candidate.py` below) if the vault is a servable git work tree with a remote; any other backend disarms it. |
| `set-base` | `<dir>` (positional) | Set the external-vault BASE directory (`~/.claude/ai-sdlc-vault-base`, tier-3), read-back-verified (slice-097). |
...
### `scripts/lib/_shard_store.py` — append-only shard store (no CLI; behind `vault_admin migrate`/`read-entries`)

The record-level storage engine (slice-088/ADR-106) sitting behind the gate-log's storage seam: an append-only shard log (one file per entry) plus a derived, git-ignored aggregate cache. `migrate()` / `read_entries()` — called from `vault_admin.py` (§above) — convert between the flat `gate-log.json` and the sharded form, and derive the cache from the shard log when it is missing (e.g. right after a `sync pull`), read-only, no write-back.

### `scripts/lib/_vault_git_sync.py` / `scripts/lib/_vault_s3_sync.py` — vault sync engines (no CLI; behind `vault_admin sync`)

The two sync engines `vault_admin.py sync push|pull` (§above) dispatches to, chosen by `--backend`. `_vault_git_sync.py` (slice-092/ADR-114) is a "sync the log, never the view" git engine: push = ensure hygiene → `git add -A` → commit → push; pull = fetch + fast-forward-only by default, REFUSING a dirty tree, a divergent history, or unpushed local commits unless `--force-drop-local`. `_vault_s3_sync.py` (slice-095/ADR-119) is the S3/MinIO twin: `resolve_config()` resolves bucket/endpoint/region/prefix from CLI args → real env vars (`AISDLC_S3_*`, §3) → the persisted `sync-backend.json` → a computed default (a machine-invariant hash of the git remote URL, or an explicit `AISDLC_S3_PROJECT`) — never importing `boto3` until a client is actually built. Both raise the same `SyncUsageError` (→ exit 2) / `SyncFailure` (→ exit 3) taxonomy that `vault_admin sync` maps onto its own exit codes.

### `scripts/lib/_sync_config.py` — persisted sync-backend config (no CLI)

The single reader/writer of `<git-common-dir>/aisdlc/sync-backend.json` (slice-097/ADR-121+123) — written by `vault_admin set-backend`, read by `vault_admin sync` (its non-secret fields are folded into `os.environ` via `setdefault` — never clobbering a real env var — before calling the S3 engine's `resolve_config` unchanged). Refuses, on both read and write, a file carrying a secret-shaped key (`AWS_ACCESS_KEY_ID` etc., or a generic `secret`/`token`/`password`/`credential` key name) or a credential embedded in an endpoint URL's userinfo (`user:pass@host`) — a credential can never enter this file.

### `scripts/lib/_claim_coord.py` / `scripts/lib/_claim_git.py` — shared-vault claim coordination (no CLI; behind `claim_candidate.py`)

The opt-in coordination primitive (slice-091/ADR-113) that makes candidate claiming safe on a shared vault: one authoritative record per candidate whose atomic create IS the winner-decision. `_claim_coord.py` defines the backend seam (`create_if_absent` / `get` / `remove_if_owner`) plus a local reference backend (`os.link`-based, one filesystem only); `_claim_git.py` (slice-100/SC-216/ADR-131+132) is the production backend for a git-synced vault — one ref per candidate, `refs/aisdlc-claims/<SC-NNN>`, made **sticky** (write-once) by pushing a parentless root commit under a `--force-with-lease=<ref>:` (empty-expect) guard, so a second concurrent create is server-side rejected rather than silently overwriting the first. Armed automatically by `vault_admin set-backend --backend git` (§above) when the vault is a servable git work tree with a remote, or by setting `AI_SDLC_CLAIM_BACKEND=git` directly (§3); disarmed by any other backend choice. `claim_candidate.py` (§below) is the only caller.
...
`prog="product_scope"`. Materializes the product's own declared scope (from `concept.json`) into `<vault>/candidates.json` (slice-068), and groups materialized capabilities into named **product areas** (slice-081/084/098). `--vault ROOT` / `--json` are accepted either before or after the verb.
...
| `add-item` | `--item-file PATH` (required, a ONE-item payload with **no** `id`), `--area NAME`, `--dry-run`, `--acknowledge LABEL` (repeatable) | Add ONE new capability to an already-persisted scope and materialize its ONE candidate — the incremental door `persist` (create-only) and `revise` (whole-list replace) never opened. The payload is composed **in-lock** from the same `cur` that is written, and a **pre-mint** `_plan` check refuses a title collision or a transitive withhold **before the id allocator runs**, so a refused add burns **zero** PS ids (a PS mint has no un-mint). `--dry-run` IS the preview and is predictive because it is the same code path: it previews **both** files (`would_add` + `would_mint`) and writes nothing. Refuses if the payload carries more than one item, carries an `id`, or would also mint candidates for pre-existing capabilities. |
...
| `set-area` | `--item PS-NNN` (required), `--area NAME` (required) | Assign ONE product-area to ONE already-materialized capability — a focused, atomic in-place annotation (mints nothing, re-materializes nothing, appends no `revisions[]` entry). `--area` is rejected if empty/whitespace or the reserved `unassigned` sentinel (case-insensitive). A same-value re-annotation is an idempotent no-op. `set-component --component NAME` is a **deprecated alias** (slice-084 renamed the product-area axis `component` → `area`, to disambiguate it from the code-component inventory below). A candidate can also carry its **own** `area` directly — overriding any area derived from this join — via `vault_edit.py update --file candidates.json --array candidates --id SC-NNN --set area=<NAME>` (slice-098/ADR-124+125; `area=null` un-annotates; a `component` key is refused on a candidate record). |

Exit codes: `0` ran (a 0-mint states its reason; `set-area`'s no-op reports `changed:false`) · `1` runtime error · `2` usage error (incl. a model-supplied id, `--scope-file` without `--dry-run`, an unknown `done`/`set-area --item` id, an empty/reserved `set-area --area`, an `add-item` payload carrying more than one item, and `add-item`'s pre-mint refusals + its `partial` state) · `3` `concept.json` absent (`decompose-context`, `persist`) · `4` `product-scope.json` absent (`materialize`, `revise`, `add-item`, `done`, `set-area`) or already-exists (`persist`).

### `scripts/lib/product_rollup.py` — per-area capability-progress rollup

`--vault ROOT` (default: resolved `VAULT_ROOT`), `--json` (default: human-readable text). A read-only DERIVED VIEW (writes nothing, slice-080 / ADR-091) — reuses `product_scope.py`'s `done` classification, joins each capability to its `product-scope` `area`, and reports done/pending counts in **capabilities** (never slices), whole-app + per-area, with a mandatory `unassigned` catch-all stratum so no capability is ever dropped. Areas are ordered least-complete-first. Emits a `governor` string only when the scope is decomposed (≥1 capability) but 0 are built — the completeness nudge `/pulse` and `/slice` render as a WARN. Exit `0` always — any compute error rides an `error` field on stdout rather than a non-zero exit. Consumed by `/pulse` (the "Product shape" line), `/slice` (the `--area` lens, via `candidates_top.py`), and `/slice-story` (via `story_inputs.py project`). Deliberately never reads a candidate's own `area` field — `area_resolve.py` (§below) is the only module that does.

### `scripts/lib/area_resolve.py` — candidate area resolution (shared library, no CLI)

The single precedence site (slice-098/SC-212, ADR-124+125) for a candidate's product **area**: a candidate's own `area` field (if valid) always wins over one derived by joining through its product-scope parent's `owner_refs`; with no own area, exactly one area-carrying parent resolves it, and 0 or 2+ parents resolve to `unassigned`. Used by `candidates_top.py --area` (§below); kept in a dedicated module so `product_rollup.py` (§above), which counts capabilities only, can never accidentally read a candidate's own fields.
...
Exit `0` ok · `2` usage error / slice not found / unreadable `--from` ledger. Consumed directly by `skills/slice-story/scripts/story_signoff.py` (§below); also runnable standalone, e.g. `$PY scripts/lib/trust_ledger.py compose --slice slice-NNN --format text`, to inspect a slice's assurance case without reading its diff.

### `skills/slice/scripts/candidates_top.py` — ranked backlog digest (with the `--area` lens + completion gate)

Single-skill tool behind `/slice`'s injected "Live state." `--top N` (default 5; `<=0` = all), `--json`, `--vault`, the optional **`--area NAME`** lens (deprecated alias `--component`, slice-080/084, population widened slice-098/ADR-125: filters to every candidate with an AREA SOURCE resolving to this area — one that asserts the area itself via its own `area` field, which overrides any derived value, or a product capability bound to it via `owner_refs`; `unassigned` selects un-grouped product capabilities; read-only, takes no lock, mints no id, writes no status; default-OFF output is byte-identical to the un-lensed digest), **`--completion-gap`** (OPT-IN, slice-102/SC-232 — classifies the pick surface for whole-app completion via `completion_gap.py` [§below] and emits the verdict, the honest done/total headline, the unbuilt-capability list, and a routed recommendation in both the text header and `--json`; read-only, default-OFF is byte-identical), and **`--explicit-intent`** (only meaningful with `--completion-gap`: the user already named the work via `/slice "<description>"`, so the gate renders its headline only and raises no question — the alert-fatigue carve-out).

### `skills/slice/scripts/completion_gap.py` — app-completion classifier (shared library, no CLI)

Not a standalone script — imported by `candidates_top.py --completion-gap` (§above). Classifies the `/slice` pick surface for whole-app **completion** (slice-102/SC-232): `product-work-available` / `scope-exhausted` (`all-built` / `none-pickable` / `empty-scope`) / `scope-absent`, plus a routed recommendation (`product-pick`, `headline-only`, or one of five `route-*` modes — `route-add-item` / `route-discover` / `route-materialize` / `route-coordinate` / `route-repair` / `route-rescope` — that HALT `/slice` into a conversation, always with a decline back to the ordinary ranked pick). Pure — no IO, no imports; every input (the rollup envelope, the orphaned set, the ranked rows) is projected by the caller. On an undecidable input the `verdict` key is omitted entirely (`{error, cause_kind}`) so a consumer branching on it fails loudly rather than reading a fabricated verdict.

### `skills/slice/scripts/claim_candidate.py` — reserve / claim / release / transfer a candidate

`prog="claim_candidate"`. Single-skill tool behind `/slice`'s claim flow (§1.3). `--candidate SC-NNN` (required) selects the target; `--vault`, `--repo-root`/`--root` (default cwd — derives the caller's git identity), `--json` are common to every mode.

| Mode | Flags | Purpose |
|---|---|---|
| claim (default) | `--name <verb-object>` (required) | CLAIM-FIRST: mint `slice-NNN` in-lock, set `status: spiking`, record `claimed_by`/`started_at`, append `pick_log`. Same-owner-only: upgrades a prior `--reserve` hold in place rather than minting twice. |
| `--reserve` | (mutually exclusive with `--release`) | Soft HOLD: mark the candidate `reserved` (claimed-in-intent) WITHOUT minting a slice number, so a parallel `/slice` sees it in-flight. Idempotent, identity-checked; `--name` not required. |
| `--release` | (mutually exclusive with `--reserve`) | Saga compensation: revert this candidate's claim or reservation. Idempotent, identity-checked, monotonic — the slice counter is never decremented. |
| `transfer` (subcommand) | `--slice slice-NNN \| --candidate SC-NNN` (exactly one, required), `--to "<Name> <email>"` (required) | Re-mint `claimed_by` of an already-claimed candidate to a new owner, in one locked read-modify-write; logged, append-only, allocator-free (mints nothing). Open to any identified caller — a third-party transfer (naming someone other than yourself) proceeds with a loud owner-naming warning rather than a refusal (ADR-122). |

Exit `1` on unset git identity or a foreign-owned reservation/claim; `transfer` fails on an unclaimed target.
...
### `skills/slice-story/scripts/story_signoff.py` — trust-ledger-derived signoff panel

`prog="story_signoff"` (slice-086 / ADR-102), two subcommands: `project --slice slice-NNN [--vault] [--json]` (compute + print, exit-0-always — a debug view) and `inject --sections-file <story-sections.json> --slice slice-NNN [--vault]` (compute + write the `trust_signoff` block into a `story-sections.json`, run on `/slice-story`'s main thread). Mechanically derives `/slice-story`'s "Who has signed off" panel from `scripts/lib/trust_ledger.compose`'s reality-contact partition — never from the narrator — so a model can never render a model-only pass as reality-proven. Three columns: `reality_approved` (a genuine reality-gate PASS, e.g. `validate-slice: pass`), `model_approved` (any model-only review, e.g. `code-review`), `not_yet` (unchecked gaps + reality-flagged concerns, e.g. a failed regression). Exit `0` for `project` always; `inject` exits non-zero only on an I/O failure (an unreadable/unwritable `story-sections.json`) — a compute problem still injects a fail-visible `state: "unavailable"` block rather than crashing.
...
| `AI_SDLC_CLAIM_BACKEND` | `scripts/lib/_claim_coord.py::coordination_backend` | Override the opt-in shared-claim coordination backend (`local` / `git`) for the current shell/process — normally set via `<git-common-dir>/aisdlc/claim-backend`, written by `vault_admin set-backend --backend git` (§2). |
| `AISDLC_S3_BUCKET` / `AISDLC_S3_ENDPOINT` / `AISDLC_S3_REGION` / `AISDLC_S3_PREFIX` / `AISDLC_S3_PROJECT` | `scripts/lib/_vault_s3_sync.py::resolve_config` | Configure the S3/MinIO vault-sync backend (target bucket; endpoint URL — unset = AWS S3; region — default `us-east-1`; object-key prefix; a machine-invariant project id used to derive the prefix when unset). Normally set via the persisted `sync-backend.json` (`vault_admin set-backend`, §2), not by hand; real env vars still take precedence over the file. |
...
| `<git-common-dir>/aisdlc/sync-backend.json` | never git-tracked | `vault_admin.py set-backend` (via `/ai-sdlc:setup`) | Persisted, non-secret vault sync-backend config (`local`/`git`/`s3` + remote/bucket/endpoint/region/project) — read by `vault_admin.py sync`. |
| `<git-common-dir>/aisdlc/claim-backend` | never git-tracked | `vault_admin.py set-backend --backend git` (via `/ai-sdlc:setup`) | The opt-in shared-claim coordination signal (`git`) — read by `claim_candidate.py` (via `_claim_coord.py`) to decide whether to route claims through the git-backed register. Removed automatically when a non-git backend is chosen. |
| `~/.claude/ai-sdlc-vault-base` | machine-local, not repo-tracked | `vault_admin.py set-base` (via `/ai-sdlc:setup`) or hand-written | Tier-3 vault base override — changes the parent directory for every project's auto-named vault. |
