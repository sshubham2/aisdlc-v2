...
- **Phase 3 — 🔶 PARTIAL (~17/19)** · prior commit `c2285d2` (v2.19.1) did 3.4, 3.6, 3.14, 3.15, 3.16.
  Session of 2026-06-11 (commits `4e2438c`→`e2a790e`, v2.19.2→v2.19.11) added:
  ...
  - **3.17** (v2.19.8) — validate-slice fork no longer "asks" mid-fork: on a shippability regression it records failed_rows + leaves `shippability_regression.deferral: null` and returns `blocked: needs-deferral-decision`; a new **Main-thread deferral resolution** section asks post-return + appends the approved deferral via vault_edit. Schema/example gained the `deferral` field.
  - **3.9** (v2.19.9) — deleted the `date(2026,5,6)` mtime carry-over (constant + `_slice_is_carry_over` + exempt branch) from the 5 remaining audits (triage_audit, critique_review_audit, exploratory_charter_audit, build_checks_audit, validate_slice_layers); `--no-carry-over` kept as a no-op; stale docstrings + critique/SKILL.md prose cleaned. Verified zero `2026, 5, 6` / `_RELEASE_DATE` repo-wide.
  - **3.19.3** (v2.19.9) — fixed finding_dedup.py's "SKETCH v0 (not yet wired)" header → LIVE, named the 2 call sites.
  - **3.11** (v2.19.11) — shrank `skill_vault_write_safety_audit.py` 733→216 lines (~140 code): deleted the directive-verb NLP lexicons, negation lookback, op-class/route-class inference, exemption-count pins. Now a dumb ADVISORY raw-write tripwire (raw-write token + shared basename, cleared by same-line vault_edit/safe_/exemption or a prohibition; exits 0 by default, `--strict` exits 1). Removed the dead SVW-1 STOP gate from validate-slice (ran with no `--root` → scanned the plugin, 1.5-class); SVW-1 is now CI-only via `.build/plugin_self_audits.py`. Added a raw-write+same-line-clear filter beyond the plan's bare heuristic (which flagged ~95 benign reads = alert-fatigue); cut to 5, still catches a real `Overwrite candidates.json`. README table updated.
  - **REMAINING:** **3.7** (merge test_first/walking_skeleton/exploratory_charter → `brief_variants_audit.py` — RISKY ~970-line consolidation; recommend AFTER the 4.4 test harness exists); **3.1 rest** (DCE-1 drift-log-timestamp cross-check needs a NEW /drift-check side-effect file → scope creep, OR demote/gate-log; BCSG-1 demote-to-advisory weakens a Critical-rule gate → **judgment call, confirm before doing**); **3.18** (schema/contract batch — incl `artifact_lint.py`, `_index.json` two shapes, examples carry load-bearing fields, **3.18.3 NEEDS USER DECISION** [variants producer-vs-delete], pulse updated_at→at, actor example, spike_ref format, + the WS-1 runnable-`verification` example polish); **3.19 rest** (mock_budget JS gap, diagnose PERSISTING bug, write-path residuals, vault_root lazy, assemble split, UX-2 preamble, UX-8 hook stderr).
